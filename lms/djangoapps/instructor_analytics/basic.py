...
def get_response_state(response):
    ...
    """
    def get_transformer():
        problem_transformers = {'openassessment': transform_ora_state, 'problem': transform_capa_state}
        problem_type = response.module_type
        return problem_transformers.get(problem_type)

    response_state = response.state
    state_transformer = get_transformer()
    if not state_transformer:
        return response_state

    state = json.loads(response_state)
    try:
        transformed_state = state_transformer(state)
